...
import org.firstinspires.ftc.teamcode.kotlinKit.routines.groups.ParallelRoutineGroup

@Disabled
@Deprecated("TESTED AND WORKING")
@TeleOp(name = "Test Routine Groups: Parallel", group = "[T] Testing KotlinKit Routines")
class ParallelRoutineGroupTest : OpMode(
    enableRoutineLogTelemetry = true
) {

    class TestRoutine : Wait(1000.0, hiddenInLogs = false)
    class TestRoutineGroup : ParallelRoutineGroup(
        TestRoutine(),
        TestRoutine(),
        TestRoutine()
    )
